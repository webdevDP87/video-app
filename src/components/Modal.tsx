...
export const Modal = (props: selectedMediaProps) => {
    ...
    const { x, isVisible, isUpscaled, setUpscaled, position, type, myList, setMyList } = props;

    if (x) {
        genres = allGenres.filter((obj: {id: number, name: string}) => x.genre_ids.includes(obj.id));
        mediaType = (type === 'movie') ? 'movie' : 'tv';
        mediaURL = (x.title || x.name).replace(/[^\w\s-]/g, '').replace(/\s+/g, '-').toLowerCase();
        fetchVideosUrl = `//api.themoviedb.org/3/${mediaType}/${x.id}/videos?api_key=${apiKey}`;
    }

    const { data: queryMediaSimilar, refetch } = useQuery([`getSimilar`], () => {
        if(x) {
            return Axios.get(`//api.themoviedb.org/3/${type}/${x.id}/similar?language=en&api_key=${apiKey}`).then((res) => {
    ...
    const { data: queryMediaGenre } = useQuery([`getGenre`], () => {
        return Axios.get(`//api.themoviedb.org/3/genre/${type}/list?language=en&api_key=${apiKey}`).then((res) => {
    ...
    useEffect(() => {
        document.body.classList.toggle('overflow-hidden', isUpscaled);
        document.body.classList.toggle('h-screen', isUpscaled);
    }, [isUpscaled]);
    ...
    }, [x, queryMediaSimilar, refetch]);

    return (
        <>
        {x && (    
                <div ref={modalWrapperRef} className={`bg-neutral-900/50 transition-colors duration-1000 static inset-0 z-20 h-0 overflow-y-auto pointer-events-none ${isUpscaled && '!h-screen !pointer-events-auto bg-neutral-900/90 !fixed'}`}>
                    <div ref={modalContainerRef} className={`py-8 transition-all duration-1000 will-change-[padding] flex justify-center ${isUpscaled && 'py-[27.5rem]'}`}>
                        <div
                            ref={modalRef}
                            ...
                            className={`card__modal z-10 group after:shadow-[0px_0px_16px_rgba(0,0,0,0.5)] after:-z-[1] after:absolute after:rounded-md after:transition-opacity after:duration-500 after:will-change-[opacity, transform] after:inset-0 rounded-md fixed top-0 left-0 right-0 will-change-[opacity] text-xs transition-all duration-500 origin-center ${isVisible || isMouseOver
                                ...
                                } ${isUpscaled ? "scale-150 !opacity-100 !static !w-[520px] pointer-events-auto" : ""}`}
                            style={position && { top: position.top, left: position.left, width: position.width, position: 'absolute' }}
                        >
                        <div className="flex flex-col">
                                <NavLink
                                    to={`/watch/${mediaURL}`}
                                    state={{ title: x.title || x.name, fetchVideosUrl: fetchVideosUrl }}
                                    className="card__video relative rounded-t-md overflow-hidden"
                                >
                                <div className="card__video__player bg-center bg-cover relative" style={{ backgroundImage: x.backdrop_path ? `url(https://image.tmdb.org/t/p/w780/${x.backdrop_path})` : 'none' }}>
                                    <div className="flex flex-col justify-end aspect-video bg-gradient-to-t from-black/75 via-black/10 to-black/0 p-4 text-base font-bold">
                                        <h2 className="opacity-00 hiddenn text-white">{x.title || x.name}</h2>
                                            <TrailerPlayer
                            ...
                            <div className="card__info p-4 rounded-b-md bg-neutral-900 flex flex-col gap-y-3">
                                <div className="card__info__user flex gap-x-2 justify-between">
                                    <div className="flex gap-x-2">
                                        ...
                                        <button onClick={() => { setMyList([...myList, x])}} className="text-white w-7 h-7 flex items-center justify-center border border-neutral-300 rounded-full transition-colors duration-300 hover:border-white hover:bg-neutral-800">
                                    ...
                                    <div className="flex gap-x-2">
                                        <button onClick={() => setUpscaled((isUpscaled: boolean) => !isUpscaled)} className="text-white w-7 h-7 flex items-center justify-center border border-neutral-300 rounded-full transition-colors duration-300 hover:border-white hover:bg-neutral-800">
                                ...
                                <div className="card__info__generes">
                                    ...
                                    {genres && (
                                        ...
                                        <ul className="flex flex-wrap text-neutral-300 font-semibold text-[11px]">
                                            {genres.map((genre: {name: string}, index: number) => {
                                                return (
                                                    <li className='flex items-center last:after:hidden after:block after:bg-neutral-600 after:mx-2 after:h-1 after:w-1 after:rounded-full' key={index}>{genre.name}</li>
                                                )
                                            })}
                                        </ul>
                                    ...
                                    {(x.overview && isUpscaled) && (
                                        <div className='mt-8 font-normal text-white'>{x.overview}</div>
                                    )}
                                        ...
                                        <>
                                            ...
                                            <ul className="grid grid-cols-4 gap-x-4 gap-y-8 !!hidden mt-4">
                                                { similar.map((item: any, index: number) => {
                                                    // const itemLink = (item?.title || item?.name)?.replace(/[^\w\s-]/g, '').replace(/\s+/g, '-').toLowerCase();
                                                    ...
                                                    return (
                                                        <li key={index} className="col-span-1">
                                                            {item && (
                                                                // <NavLink to={`/watch/${itemLink}`} state={{ title: item?.name, fetchVideosUrl: fetchVideosUrl }} className="group transition duration-500 hover:text-white">
